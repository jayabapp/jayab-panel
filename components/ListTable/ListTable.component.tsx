...
import { ActionButton, Column, CreateProps, FilterProp, TableServerProps } from "@/components/Table/table.type";
...
const ListTable = ({
  ...
  }, [currentPage, queriesParams, mustRefresh]);
  ...
  const createTableActions = (row: any) => {
      ...
      switch (action) {
        case "show":
          items.push({
            id: 1,
            component: (
              <ShowButton onPress={() => router.push(`/${pluralize(kebabCase(model) || "")}/show/${row.id}`)} />
            ),
          });
          break;
    ...
    items
      ...
      //       onPress={() =>
      //         router.push(
      //           `/content-questions?filters=filters[content_id][equals]=${row.id}`
      //         )
      //       }
      //     >
      //       سوالات
