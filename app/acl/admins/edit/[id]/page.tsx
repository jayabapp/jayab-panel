...
const AdminEdit = () => {
  ...
  const getRoles = () => {
    ApiCall("GET", apiRoutes.ACL_ROLES, null, "GET  ROLES", ({ data }) => {
      ...
      if (queriesParams?.business_id) {
        // const r = data.find((e: any) => e.key == AdminRole.BUSINESS_OWNER);
        // setStates((e) => ({ ...e, role: r }));
        setNotEditableList(["role"]);
      }
      getAdmin();
    });
  };
  ...
  const getAdmin = () => {
    ApiCall(
      ...
      "GET  ADMIN INFO",
      ({ data }) => {
        const role = initFormItems[0].selectItems?.find(
          (e) => e.id == data.role_id,
        );
