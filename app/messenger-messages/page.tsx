...
import ExcelExportModal from "@/components/Table/ExcelExportModal";
...
const MessengerMessages = ({}) => {
  ...
  const chatroomId = queriesParams?.chatroom_id;
  ...
  /* -------------------------------- GET LIST -------------------------------- */
  const getList = (page: number, filterQuery: object, isExcel?: boolean) => {
    ...
    ApiCall(
      "GET",
      isExcel ? `${endpoint}/excel` : endpoint,
      body,
      "GET LIST",
      ({ data }) => {
        if (!isExcel) {
  ...
  const onSubmitEditedRow = (row: any) => {
    ...
    editableColumns.map((key) => {
      if (typeof updatedStates[rowId]?.[key] == "boolean")
        body[key] = updatedStates[rowId]?.[key];
      else
        body[key] =
          updatedStates[rowId]?.[key]?.id ||
          updatedStates[rowId]?.[key] ||
          row[key];
    });
      ...
      items.push({
        ...
        component: (
          <Button
            ...
            variant="solid"
            size="sm"
            startContent={<ChatsCircle className="w-4" />}
            className="w-auto font-medium"
            onPress={() =>
              router.push(`/messenger-messages?chatroom_id=${row.chatroom_id}`)
            }
          >
            همه‌ی پیام‌ها
          </Button>
